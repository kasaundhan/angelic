      ...
      <form role="form" action="{{url('quote/update')}}" method="post" id="quote_update" class="form-validate" data-parsley-validate >
            ...
            <table class="table table-bordered" border="1">
              <tr class="row-temp">
                <td><div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Item code" name="itm_code[]" value="{{$quoteitems->item_code}}" >
                  </div></td>
                <td><div class="form-group">
                    <label>Metal </label>
              ...
              <td rowspan="2" style="vertical-align:middle"><div class="btn-group">
                 @if($i==1)<button type="button" id="add_row" class="btn btn-primary btn-sm add-row" title="Add New Row"><i class="fa fa-fw fa-plus"></i></button>@endif
                    <button type="button" class="btn btn-danger btn-sm remove-row" title="Remove Row" id="{{$quoteitems->id}}"><i class="fa fa-fw fa-minus"></i></button>
                  </div></td>
              </tr>
              <tr class="row-temp2">
                <td><div class="form-group">
        ...
        <div class="col-md-12">
          <div class="form-group">
           ...
           <textarea  class="form-control" name="staff_note" placeholder="Staff Notes">@if(!empty($quoteitem[0]->staff_note)){{$quoteitem[0]->staff_note}} @endif
           </textarea>
          </div>
...
/*	$("#customer_email").autocomplete({
	...
	$(document).on('click','.add-row',function(){
		...
		if($(this).parents('table').find('input[name="itm_code[]"]').val()==''){
			...
			if($('#table-list table').length<=9){
				var tablehtml = $(this).parents('table').html();
	...
	$(document).on('click','.remove-row',function(){
			...
			if($('#table-list table').length ==2){
			 $('#table-list table').find('button.add-row').css('display','block');	
			//alert($('#table-list').find('table:first').html());	
			}
			
			console.log($('#table-listtable:last').html());
			...
			if($('#table-list table').length >= 2){
			  if(confirm('Are you sure?')){
				  ...
				  if($('button.add-row:visible').length ==1){
			 $('#table-list table').find('button.add-row').css('display','block');	
			//alert($('#table-list').find('table:first').html());	
			}
